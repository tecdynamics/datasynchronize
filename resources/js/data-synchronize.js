$(() => {
    const exportFile = ($form, data, button) => {
        ...
        httpClient
            ...
            .then(({ headers, data }) => {
                ...
                Tec.showSuccess($form.data('success-message'))
            })
            .catch(() => {
                Tec.showError($form.data('error-message'))
            })
    }
    ...
    if ($form.length > 0) {
        ...
        const $output = $form.find('.datasynchronize-import-output')
        let errors = []
        let failures = []
        let total = null
        ...
        const dropzone = new Dropzone($form.find('.dropzone').get(0), {
            ...
            previewTemplate: $('#datasynchronize-import-preview-template').html(),
