...
namespace Tec\DataSynchronize\Exporter;

use Tec\Base\Facades\Assets;
use Tec\Base\Facades\BaseHelper;
use Tec\DataSynchronize\Concerns\Exporter\HasEmptyState;
use Tec\DataSynchronize\Enums\ExportColumnType;
...
abstract class Exporter implements FromCollection, ShouldAutoSize, WithColumnFormatting, WithEvents, WithHeadings, WithMapping
{
    ...
    /**
     * @return \Tec\DataSynchronize\Exporter\ExportColumn[]
     */
    ...
    {
        return trans(
            'packages/datasynchronize::datasynchronize.export.heading',
            ['label' => $this->getLabel()]
        );
    }

    /**
     * @return \Tec\DataSynchronize\Exporter\ExportCounter[]
     */
    ...
    {
        Assets::addScriptsDirectly('vendor/core/packages/datasynchronize/js/datasynchronize.js');

        return view('packages/datasynchronize::export', [
            'exporter' => $this,
        ]);
    }
