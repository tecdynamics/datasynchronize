...
namespace Tec\DataSynchronize\Importer;

use Tec\Base\Facades\Assets;
use Tec\DataSynchronize\Concerns\Importer\HasImportResults;
use Tec\DataSynchronize\Contracts\Importer\WithMapping;
use Tec\DataSynchronize\DataTransferObjects\ChunkImportResponse;
use Tec\DataSynchronize\DataTransferObjects\ChunkValidateResponse;
use Tec\DataSynchronize\Exporter\ExampleExporter;
...
abstract class Importer
{
    ...
    public function getAcceptedFiles(): array
    {
        return apply_filters(
            ...
            config('packages.datasynchronize.datasynchronize.mime_types')
        );
    }
    ...
    public function getFileExtensions(): array
    {
        return apply_filters(
            ...
            config('packages.datasynchronize.datasynchronize.extensions')
        );
    }
    ...
    {
        return trans(
            'packages/datasynchronize::datasynchronize.import.heading',
            ['label' => $this->getLabel()]
        );
    }
    ...
    {
        Assets::addStylesDirectly('vendor/core/packages/datasynchronize/css/datasynchronize.css')
            ->addScriptsDirectly('vendor/core/packages/datasynchronize/js/datasynchronize.js')
            ->addScripts('dropzone')
            ->addStyles('dropzone');

        $view = 'packages/datasynchronize::import';

        if ($this->renderWithoutLayout) {
            $view = 'packages/datasynchronize::partials.importer';
        }
    ...
    public function validate(string $fileName, int $offset = 0, int $limit = 100): ChunkValidateResponse
    {
        ...
        if ($count === 0) {
            ...
            $storageFolder = config('packages.datasynchronize.datasynchronize.storage.path');
    ...
    {
        return trans('packages/datasynchronize::datasynchronize.import.importing_message', [
    ...
    {
        return trans('packages/datasynchronize::datasynchronize.import.done_message', [
    ...
    {
        $filePath = sprintf('%s/%s', config('packages.datasynchronize.datasynchronize.storage.path'), $fileName);
    ...
    {
        return Storage::disk(config('packages.datasynchronize.datasynchronize.storage.disk'));
    }
