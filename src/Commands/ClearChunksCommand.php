...
namespace Tec\DataSynchronize\Commands;
...
#[AsCommand('cms:datasynchronize:clear-chunks', 'Remove all expired chunks')]
class ClearChunksCommand extends Command
{
    public function handle(): int
    {
        $storageDisk = Storage::disk(config('packages.datasynchronize.datasynchronize.storage.disk'));

        $storagePath = config('packages.datasynchronize.datasynchronize.storage.path');
